...
void mainImage(out vec4 O, vec2 C) {
  float
  i = 0.0 // Loop counter (starts at 0)
  , d = 0.0 // Distance to nearest surface
  , z = fract(dot(C, sin(C))) - .5 // Ray distance + noise for anti-banding
  ;
  vec4
  o = vec4(0.0) // Accumulated color/lighting
  , p = vec4(0.0) // Current 3D position along ray
  ;
  vec2 r = iResolution.xy; // Screen resolution
  for (int rayStep = 1; rayStep < 77; rayStep++) {
    i = float(rayStep);
    ...
    p.xy *= mat2(cos(2. + O.z + vec4(0.0, 11.0, 33.0, 0.0)));
    ...
    O = (1. + sin(.5 * O.z + length(p - O) + vec4(0.0, 4.0, 3.0, 6.0)))
        / (.5 + 2. * dot(O.xy, O.xy));
